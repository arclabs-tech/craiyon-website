...
export default function LiveLeaderboard() {
  ...
  const fetchLeaderboard = async () => {
    try {
      ...
      const res = await fetch('/api/leaderboard', { signal: controller.signal, cache: 'no-store' });
      ...
      <CardContent>
        ...
        <div className="space-y-3" aria-live="polite" aria-busy={loading}>
          {loading && leaderboard.length === 0 && (
            <div className="space-y-2 animate-pulse" data-testid="leaderboard-loading">
              {Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="h-16 rounded-lg bg-slate-200/60 dark:bg-slate-700/50" />
              ))}
            </div>
          )}
