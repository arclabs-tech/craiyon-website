...
async function base64ForPublicFile(relUrl: string): Promise<string> {
  try {
    ...
    console.log(
      `Successfully read local image: ${fullPath} (${data.length} bytes)`,
    );
...
async function getEmbeddingWithRetry(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.log(
        `🧠 Computing embedding for ${name} (attempt ${attempt}/${maxRetries})...`,
      );
      ...
      const embedding = await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          setTimeout(
            () => reject(new Error(`Embedding timeout after 20s`)),
            20000,
          ),
        ),
      ]);
      ...
      console.warn(
        `⚠️ Embedding attempt ${attempt}/${maxRetries} failed for ${name}:`,
        error,
      );

      if (attempt === maxRetries) {
        throw new Error(
          `Failed to get embedding for ${name} after ${maxRetries} attempts: ${error}`,
        );
      }
  ...
  try {
    ...
    if (originalImageUrl.startsWith("/")) {
      ...
      origB64 = await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          setTimeout(
            () => reject(new Error("Timeout fetching original")),
            10000,
          ),
        ),
      ]);
    ...
    const genB64 = await Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(
          () => reject(new Error("Timeout fetching generated")),
          10000,
        ),
      ),
    ]);
    ...
    console.log(
      `📊 Image sizes: orig=${origB64.length} chars, gen=${genB64.length} chars`,
    );
    ...
    console.log(
      `📐 Embeddings: orig=${origEmb.length}D, gen=${genEmb.length}D`,
    );
