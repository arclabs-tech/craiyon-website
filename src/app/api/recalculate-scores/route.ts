...
export async function POST() {
  try {
    console.log('🔄 Recalculating all user scores based on max per challenge...');
    ...
    for (const user of users) {
      ...
      const totalScore = bestScores.reduce((sum, record) => sum + (Number(record.max_score) || 0), 0);
      ...
      console.log(`✅ ${user.username}: recalculated to ${totalScore.toFixed(2)} (from ${bestScores.length} challenges)`);
    }
